...
/*
	...
	int np;
	np = 1<<(myTarget.length-1);//2^(n-1) n:target.length //npは比較する項の数
    ...
    for( int i = 0 ; i < myTarget.length ; i++ ){
        for( int j = 0 ; j <= i ; j++ ){
            if(j == 0){
				...
				//System.out.println("value1 = "+value1 +"Iq["+i+"] = "+ Iq[i]);
